...
export default function InstagramContent() {
  ...
  const handleLike = async (postId: number) => {
    ...
    try {
      ...
      setLikesMap((m) => ({ ...m, [postId]: { count: newCount, likedByMe: nowLiked } }));
      if (nowLiked) {
  ...
  const handleCommentLike = async (commentId: string) => {
    ...
    try {
      ...
      setCommentLikesMap((m) => ({
        ...m,
        [commentId]: { count: nowLiked ? prev.count + 1 : prev.count - 1, likedByMe: nowLiked },
      }));
    } catch {
      // ignore
    } finally {
        ...
        <div className="insta-dialog-overlay">
          <div className="insta-dialog">
            <div className="insta-dialog-title">
              ...
              <button type="button" className="insta-dialog-close" onClick={() => setShowDialog(false)}>X</button>
            </div>
            ...
            <div className="insta-dialog-buttons">
              <button type="button" onClick={() => setShowDialog(false)}>OK</button>
      ...
      {currentView === 'post' && selectedPost && (
        <>
          <div className="insta-nav-top">
            <button type="button" className="insta-nav-btn" onClick={handleBack}>Back</button>
            <PixelIcon src={iconSun} alt="brightness" />
            <button type="button" className="insta-nav-btn" onClick={() => setCurrentView('filter')}>Next</button>
          </div>
          <div className="insta-post-view">
            <img className="insta-post-image" src={selectedPost.img} alt="post" />
          </div>
          <div className="insta-post-actions">
            <button type="button" className="insta-action-btn" onClick={() => handleLike(selectedPost.id)} disabled={!nickname || toggling}>
              <PixelIcon src={iconHeart} alt="like" className={getLikeInfo(selectedPost.id).likedByMe ? 'insta-icon-liked' : ''} />
            </button>
            <button type="button" className="insta-action-btn" onClick={() => handleOpenComments(selectedPost)}>
          ...
          <div className="insta-post-info">
            <span className="insta-post-likes">{getLikeInfo(selectedPost.id).count} likes</span>
            <span className="insta-post-caption">{selectedPost.caption}</span>
            {(commentCounts[selectedPost.id] ?? 0) > 2 && (
              <button type="button" className="insta-view-comments" onClick={() => handleOpenComments(selectedPost)}>
                View all {commentCounts[selectedPost.id]} comments
              </button>
            )}
            {previewComments.length > 0 && (
              <div className="insta-comment-preview" onClick={() => handleOpenComments(selectedPost)}>
            ...
            {!nickname && <span className="insta-post-hint">* Log in to like & comment</span>}
          </div>
          <div className="insta-bottom-bar">
            <button type="button" onClick={handleBack}>Filter</button>
            <button type="button" onClick={() => setCurrentView('filter')}>Edit</button>
          </div>
        </>
      )}
      ...
      {currentView === 'filter' && selectedPost && (
        <>
          <div className="insta-nav-top">
            <button type="button" className="insta-nav-btn" onClick={handleBack}>Back</button>
            <PixelIcon src={iconSun} alt="brightness" />
            <button type="button" className="insta-nav-btn" onClick={() => setCurrentView('profile')}>Next</button>
          </div>
          <div className="insta-post-view">
            <img className="insta-post-image" src={selectedPost.img} alt="post" style={{ filter: FILTERS[activeFilter].style }} />
          </div>
          <div className="insta-filter-strip">
            {FILTERS.map((f, i) => (
              <div key={f.name} className={`insta-filter-item ${i === activeFilter ? 'insta-filter-active' : ''}`} onClick={() => setActiveFilter(i)}>
                <span className="insta-filter-label">{f.name}</span>
                <div className="insta-filter-thumb">
                  <img src={selectedPost.img} alt={f.name} style={{ filter: f.style }} />
                </div>
              </div>
            ))}
          </div>
      ...
      {currentView === 'comments' && selectedPost && (
        <>
          ...
          <div className="insta-comments-list">
            {comments.length === 0 && <div className="insta-comments-empty">No comments yet.</div>}
            {comments.map((c) => {
              ...
              return (
                <div key={c.id} className="insta-comment-item">
                  <div className="insta-comment-header">
                    <span className="insta-comment-nickname">{c.nickname}</span>
                    <span className="insta-comment-time">{new Date(c.created_at).toLocaleDateString()}</span>
                  </div>
                  <div className="insta-comment-body">
                    <span className="insta-comment-content">{c.content}</span>
                    <div className="insta-comment-actions">
                      <button type="button" className="insta-comment-like-btn" onClick={() => handleCommentLike(c.id)} disabled={!nickname || togglingCommentLike}>
                        <PixelIcon src={iconHeart} alt="like" className={cLike.likedByMe ? 'insta-icon-liked' : ''} />
                      </button>
                      {cLike.count > 0 && <span className="insta-comment-like-count">{cLike.count}</span>}
                      {c.nickname === nickname && (
                        <button type="button" className="insta-comment-delete" onClick={() => handleDeleteComment(c.id)}>X</button>
                      )}
                    </div>
                  </div>
                </div>
          ...
          <div className="insta-comment-input-area">
            <input
              ...
              maxLength={200}
            />
            <button type="button" className="insta-comment-submit" onClick={handleSubmitComment} disabled={!nickname || !commentText.trim() || submitting}>
              Post
            </button>
          </div>
        </>
      ...
      {currentView === 'profile' && (
        <>
          ...
          <div className="insta-tabs">
            <button type="button" className={`insta-tab ${activeTab === 'grid' ? 'insta-tab-active' : ''}`} onClick={() => setActiveTab('grid')}>
              <PixelIcon src={iconGrid} alt="grid" />
            </button>
            <button type="button" className={`insta-tab ${activeTab === 'list' ? 'insta-tab-active' : ''}`} onClick={() => setActiveTab('list')}>
              <PixelIcon src={iconList} alt="list" />
            </button>
            <button type="button" className={`insta-tab ${activeTab === 'reels' ? 'insta-tab-active' : ''}`} onClick={() => setActiveTab('reels')}>
              <PixelIcon src={iconPlay} alt="reels" />
            </button>
            <button type="button" className={`insta-tab ${activeTab === 'tagged' ? 'insta-tab-active' : ''}`} onClick={() => setActiveTab('tagged')}>
          ...
          <div className="insta-grid">
            {POSTS.map((post) => {
              ...
              return (
                <div key={post.id} className="insta-grid-item" onClick={() => handlePostClick(post)}>
                  <img src={post.img} alt={`post-${post.id}`} loading="lazy" decoding="async" />
                  {(info.count > 0 || cCount > 0) && (
                    <span className="insta-grid-likes">
                      {info.count > 0 && (<><PixelIcon src={iconHeart} alt="likes" className="insta-icon-liked" />{info.count}</>)}
                      {cCount > 0 && (<><PixelIcon src={iconComment} alt="comments" />{cCount}</>)}
                    </span>
                  )}
                </div>
              );
          ...
          <div className="insta-bottom-nav">
            <button type="button" className="insta-bnav-btn insta-bnav-active"><PixelIcon src={iconHome} alt="home" /></button>
            <button type="button" className="insta-bnav-btn"><PixelIcon src={iconSearch} alt="search" /></button>
            <button type="button" className="insta-bnav-btn"><PixelIcon src={iconCamera} alt="camera" /></button>
            <button type="button" className="insta-bnav-btn"><PixelIcon src={iconHeart} alt="activity" /></button>
            <button type="button" className="insta-bnav-btn"><PixelIcon src={iconUser} alt="profile" /></button>
          </div>
        </>
      )}
    </div>
