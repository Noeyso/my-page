...
const FALLBACK_NEWS: Record<string, NewsItem[]> = {
  ...
  경제: [
    { title: '증시 동향과 경제 뉴스', link: '#', pubDate: '', source: 'Yahoo' },
  ],
  테크: [
    { title: 'IT/테크 트렌드', link: '#', pubDate: '', source: 'Yahoo' },
  ],
  생활: [
    { title: '생활/문화 소식', link: '#', pubDate: '', source: 'Yahoo' },
  ],
};
...
export default function YahooContent() {
  ...
  const isLoading = activeCategory === '홈'
      ...
      <div className="yahoo-page">
        ...
        <div className="yahoo-header">
          <div className="yahoo-logo-area">
            ...
            <div className="yahoo-search-area">
              ...
              <div className="yahoo-search-row">
                ...
                />
                <button type="button" className="yahoo-search-btn" onClick={handleSearch}>검색</button>
              </div>
          ...
          <div className="yahoo-top-links">
            <span onClick={() => handleNavigate('뉴스')} style={{ cursor: 'pointer' }}>오늘의뉴스</span>
            <span onClick={() => handleNavigate('연예')} style={{ cursor: 'pointer' }}>연예</span>
            <span onClick={() => handleNavigate('스포츠')} style={{ cursor: 'pointer' }}>스포츠</span>
          </div>
        </div>
              ...
              <div className="yahoo-news-section">
                ...
                {currentNews.map((item, i) => (
                  <div key={i} className="yahoo-news-item">
                    <span className="yahoo-news-bullet" style={{ color: i < 3 ? '#cc0000' : '#1a0dab' }}>●</span>
                    <a
                      href={item.link}
                      target="_blank"
                      rel="noopener noreferrer"
                      className="yahoo-news-link"
                    >
                      {item.title}
                    </a>
  ...
  return (
    <div className="yahoo-news-section">
      <div className="yahoo-category-header">
        {title}
        {isLoading && <span className="yahoo-loading-inline">로딩중...</span>}
        <span className="yahoo-more-link" onClick={onMore}>더보기 →</span>
      </div>
      {items.map((item, i) => (
        <div key={i} className="yahoo-news-item">
          <span className="yahoo-news-bullet" style={{ color: bulletColor }}>●</span>
          <a
            href={item.link}
            target="_blank"
            rel="noopener noreferrer"
...
const GG_CATEGORIES = [
  {
    label: '게임', color: '#4477cc',
    items: [
      { name: '인형놀이', icon: '🎎' }, { name: '변신슈가', icon: '🧁' },
      { name: '아비스모험', icon: '⚔️' }, { name: '월드슬레이어', icon: '🌍' },
      { name: '대결짱', icon: '🥊' }, { name: '마법학교', icon: '🧙' },
    ],
  },
  {
    label: '재미', color: '#dd5599',
    items: [
      { name: '만화', icon: '📖' }, { name: '캐릭터극장', icon: '🎭' },
      { name: '재미플러스', icon: '🎪' }, { name: '타자왕', icon: '⌨️' },
      { name: '라이벌', icon: '🏆' }, { name: '만화TV', icon: '📺' },
    ],
  },
  {
    label: '학습', color: '#339944',
    items: [
      { name: '숙제박사', icon: '📝' }, { name: '두뇌왕', icon: '🧠' },
      { name: '초등학습', icon: '📚' }, { name: '에듀모아', icon: '🎓' },
      { name: '공부방', icon: '🏫' }, { name: '꾸러기사전', icon: '📕' },
    ],
  },
  {
    label: '테마', color: '#ee8833',
    items: [
      { name: '연예인+', icon: '⭐' }, { name: '캐릭터+', icon: '🐻' },
      { name: '동영상+', icon: '🎬' }, { name: '인기', icon: '🔥' },
      { name: '괴퍼', icon: '👾' }, { name: 'X마스', icon: '🎄' },
      { name: '원더걸스', icon: '🎤' }, { name: '빅뱅', icon: '🎸' },
    ],
  },
];

const GG_CHARACTERS = [
  { name: '뽀로로', icon: '🐧' }, { name: '올톨', icon: '🐣' },
  { name: '캐비', icon: '🐰' }, { name: '공룡', icon: '🦕' },
  { name: '포밍', icon: '🐱' }, { name: '싱크', icon: '🐬' },
  { name: '슈', icon: '🧚' }, { name: '쿵쿵', icon: '🐻' },
  { name: '콘콘', icon: '🦊' }, { name: '엄마아이', icon: '👩‍👧' },
  { name: '네티캣', icon: '🐈' },
];

const GG_TODDLER = [
  { label: '푸키하우스', icon: '🏠' }, { label: '유아놀이', icon: '🧸' },
  { label: '동요나라', icon: '🎵' }, { label: '동화나라', icon: '📖' },
  { label: '영어친구', icon: '🔤' }, { label: '유치원', icon: '🏫' },
  { label: '유아TV', icon: '📺' },
];
...
const GAMES: GameInfo[] = [
  { id: 'g1', name: '드래곤 어드벤처', category: '모험/액션', icon: '🐉', color: '#4488cc', desc: '용과 함께 신비로운 세계를 모험하는 RPG 게임! 다양한 스킬과 아이템을 수집하며 보스를 물리치자!', rating: 4.5, plays: '12,345' },
  { id: 'g2', name: '스타슈터', category: '슈팅', icon: '🚀', color: '#cc4444', desc: '우주를 배경으로 한 슈팅게임. 적 우주선을 물리치고 은하계를 지켜라!', rating: 4.2, plays: '8,912' },
  { id: 'g3', name: '축구왕 슛돌이', category: '스포츠', icon: '⚽', color: '#44aa44', desc: '신나는 축구 게임! 슛을 날려 골을 넣어라! 월드컵 우승에 도전!', rating: 4.0, plays: '15,678' },
  { id: 'g4', name: '보석 퍼즐', category: '보드/퍼즐', icon: '💎', color: '#aa44aa', desc: '반짝이는 보석을 3개 이상 맞춰서 없애는 중독성 퍼즐 게임!', rating: 4.8, plays: '25,100' },
  { id: 'g5', name: '냥이 대모험', category: '모험/액션', icon: '🐱', color: '#ff8844', desc: '귀여운 고양이의 모험! 점프하고 달리며 물고기를 모아라!', rating: 4.3, plays: '9,876' },
  { id: 'g6', name: '쿠킹 마스터', category: '캐주얼', icon: '🍳', color: '#ee7744', desc: '요리사가 되어 맛있는 음식을 만들어보자! 다양한 레시피에 도전!', rating: 4.1, plays: '7,543' },
  { id: 'g7', name: '구구단 챔피언', category: '교육', icon: '🔢', color: '#44aacc', desc: '구구단을 빠르게 풀어 챔피언에 도전! 수학 실력을 높여보자!', rating: 3.9, plays: '6,234' },
    ...
    <div className="gg-page">
      ...
      <div className="gg-header">
        <div className="gg-header-top">
          <button type="button" className="gg-back-btn" onClick={onBack}>← Yahoo!</button>
          <div className="gg-logo">
            <span className="gg-logo-yahoo">YAHOO!</span>
            <span className="gg-logo-title">꾸러기</span>
          </div>
          <div className="gg-subtitle">인터넷친구 ♪ 너희들의꾸러기!</div>
        </div>
        {/* Character banner */}
        <div className="gg-banner-chars">
          <span className="gg-banner-char">🧒</span>
          <span className="gg-banner-char">👧</span>
          <span className="gg-banner-char">🐻‍❄️</span>
          <span className="gg-banner-char">🐧</span>
          <span className="gg-banner-char">🎄</span>
          <span className="gg-banner-char">⭐</span>
        </div>
      ...
      <div className="gg-content">
        {GG_CATEGORIES.map((cat) => (
          <div key={cat.label} className="gg-row">
            <div className="gg-row-label" style={{ background: cat.color }}>{cat.label}</div>
            <div className="gg-row-items">
              {cat.items.map((item) => (
                <div key={item.name} className="gg-item">
      ...
      <div className="gg-search-bar">
        ...
        <button type="button" className="gg-search-btn-k">검색</button>
      </div>
        ...
        <div className="gg-body-side">
          <div className="gg-side-box">
            ...
            <div className="gg-side-list">
              <div className="gg-side-item"><span className="gg-rank">1</span> 뽀로로의 모험</div>
              <div className="gg-side-item"><span className="gg-rank">2</span> 마법천자문</div>
              <div className="gg-side-item"><span className="gg-rank">3</span> 코난 탐정</div>
              <div className="gg-side-item"><span className="gg-rank">4</span> 원피스 항해</div>
              <div className="gg-side-item"><span className="gg-rank">5</span> 짱구는 못말려</div>
            </div>
          </div>
          <div className="gg-side-box">
          ...
          {['게임', '학습', '재미', '만화', '캐릭터', '동영상'].map((s) => (
            <div key={s} className="gg-more-col">
              ...
              <div className="gg-more-col-items">
                <span>인기순위</span><span>최신목록</span><span>추천</span>
              </div>
            </div>
          ))}
        </div>
...
function GamePlusPage({
  ...
  const filteredGames = category === '인기게임'
    ? GAMES
    : GAMES.filter((g) => g.category === category);

  const tabGames = activeTab === '인기게임'
    ? filteredGames.slice().sort((a, b) => parseFloat(b.plays.replace(/,/g, '')) - parseFloat(a.plays.replace(/,/g, '')))
    : activeTab === '최신게임'
    ...
    <div className="gp-page">
      ...
      <div className="gp-header">
        <div className="gp-breadcrumb">
          <span className="gp-bc-link" onClick={onBack}>꾸러기홈</span>
    ...
    <div className="gp-page">
      ...
      <div className="gp-header">
        <div className="gp-breadcrumb">
          <span className="gp-bc-link" onClick={onBack}>꾸러기홈</span>
          <span className="gp-bc-sep">&gt;</span>
          <span className="gp-bc-link" onClick={onBack}>게임플러스</span>
          <span className="gp-bc-sep">&gt;</span>
